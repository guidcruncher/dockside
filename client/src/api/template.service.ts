...
export class TemplateClient extends BaseClient {
  ...
  getTemplate(name: string): Promise<Template> {
    return this.request(`/api/v1/templates/${encodeURIComponent(name)}`)
  }

  compose(name: string): Promise<any> {
    return this.request(`/api/v1/templates/${encodeURIComponent(name)}/compose`)
  }
}
