...
import type { DockerContainerStats, DockerContainerSummary } from "@/api"
...
export function useContainers() {
  ...
  async function stats(id: string): Promise<DockerContainerStats> {
    try {
      return await docksideClient.containers.stats(id)
    } catch (e) {
  ...
  async function statsList(id: string[]): Promise<DockerContainerStats[]> {
    try {
      return await docksideClient.containers.statsList(id)
    } catch (e) {
      toasts.error(e instanceof Error ? e.message : String(e))
      throw e
    }
