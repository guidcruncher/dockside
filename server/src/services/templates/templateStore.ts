...
export class TemplateStore {
  ...
  async getLocalTemplate(name: string): Promise<Template | null> {
    ...
    try {
      ...
      logger.error(err, `Failed to load local template ${name}:`)
      return null
    }
  }
}
...
export const runTemplateSync = async (store: TemplateStore) => {
  try {
    ...
    logger.info(`Found ${syncKeys.length} updates. Starting downloads...`)
    ...
    results.forEach((result, index) => {
      if (result.status === "fulfilled") {
        logger.info(`Successfully synced: ${syncKeys[index]}`)
      } else {
        logger.error(`Failed to sync ${syncKeys[index]}: ${result.reason}`)
      }
    })
    ...
    logger.error(error, "Critical error during sync:")
  }
}
