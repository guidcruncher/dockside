...
import { SystemMonitor } from "#/services/system/systemMonitor.js"
...
export const diPlugin = fp(async (fastify) => {
  ...
  const portainerTemplateStore = new PortainerTemplateStore(config)
  const systemMonitor = new SystemMonitor()
  const proxyService = new ProxyService(config, persistentWorkspaceStore)

  Promise.allSettled([
    persistentWorkspaceStore.setWorkspaceRoot(),
    runTemplateSync(templateStore),
    portainerTemplateStore.load(),
  ])
