...
export class PersistentWorkspaceStore {
  ...
  public findByName(projectName: string): any {
    for (const key of Object.keys(this.state.projects)) {
      const proj = this.state.projects[key]
      if (!proj) continue
  ...
  private async saveToDisk(): Promise<void> {
    ...
    try {
      ...
      logger.error(error, "Failed to write workspace state to disk:")
    }
  }
  ...
  private async loadFromDisk(): Promise<PersistedWorkspaceData | null> {
    ...
    try {
      ...
      logger.error(error, "Failed to read or parse workspace state from disk:")
      return null
    }
  }
